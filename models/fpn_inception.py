...
class FPNInception(nn.Module):
    ...
    def forward(self, x):
        ...
        map4 = torch.nn.functional.interpolate(self.head4(map4), scale_factor=8, mode="nearest")
        map3 = torch.nn.functional.interpolate(self.head3(map3), scale_factor=4, mode="nearest")
        map2 = torch.nn.functional.interpolate(self.head2(map2), scale_factor=2, mode="nearest")
        map1 = torch.nn.functional.interpolate(self.head1(map1), scale_factor=1, mode="nearest")

        smoothed = self.smooth(torch.cat([map4, map3, map2, map1], dim=1))
        smoothed = torch.nn.functional.interpolate(smoothed, scale_factor=2, mode="nearest")
...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        pad1 = (0, 1, 0, 1)
        map4 = lateral4
        map3 = self.td1(lateral3 + torch.nn.functional.interpolate(map4, scale_factor=2, mode="nearest"))
        map2 = self.td2(F.pad(lateral2, pad, "reflect") + torch.nn.functional.interpolate(map3, scale_factor=2, mode="nearest"))
        map1 = self.td3(lateral1 + torch.nn.functional.interpolate(map2, scale_factor=2, mode="nearest"))
